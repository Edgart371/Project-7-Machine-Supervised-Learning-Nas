...
neonew.to_csv(r'C:\Users\leube\Ironhack\Ironprojects\Module_3\Project7\cleaned_prepped_data.csv')
...
# attempt to make models more precise

x = neonew.drop('hazardous', axis=1)
y = neonew['hazardous']
x_train,x_test,y_train,y_test = train_test_split(x,y, test_size=0.3, random_state=42)

ones = neonew[neonew['hazardous']==1]
len1 = len(ones)

zeros = neonew[neonew['hazardous']==0]


zerosam = zeros.sample(n=len1, random_state=42)

zerosam.columns == ones.columns
allalt = pd.concat([ones, zerosam])

# setiing up new train and test data

x = allalt.drop('hazardous', axis=1)
y = allalt['hazardous']
x_train,x_test,y_train,y_test = train_test_split(x,y, test_size=0.3, random_state=42)
...
# random forest +1111111

model = RandomForestClassifier(max_features=None,min_samples_leaf=1,n_estimators=110)
result = model.fit(x_train,y_train)
prediction = result.predict(x_test)
...
# altering the data or to be more precious the train and test data in this way
# has produced more accurate results



# check of this model with random hazardous sample
checksam = neonew.sample(n=len(x_test), random_state=42)
x_sam = checksam.drop('hazardous', axis=1)
y_sam = checksam['hazardous']


model = AdaBoostClassifier(n_estimators=160)
result = model.fit(x_train,y_train)
prediction = result.predict(x_sam)

print(classification_report(y_sam,prediction))
generate_results(prediction,y_sam,model,x_sam)
...
# checking if outliers have an impact on the results of the test
numcol.remove('hazardous')
numcol
...
for x in numcol:
    ...
    Q1= neonew[x].quantile(0.25)
    IQR = Q3 - Q1
    neonew.drop(neonew[neonew[x]>Q3 + 1.5*IQR].index,inplace=True)
    neonew.drop(neonew[neonew[x]<Q1 - 1.5*IQR].index,inplace=True)
...
# test if deleted rows have an impact
# ada boost
...
# differences of scores in some cases, improvement, but not remarkable difference

# --> outliers not responsible for low recall and precision score of for true(hazardous) comets
